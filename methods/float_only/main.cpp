...
std::vector<BatchTokens> build_single_sequence_batches(
    ...
    batches.reserve(static_cast<std::size_t>(doc_count));
    for (int doc_idx = 0; doc_idx < doc_count; ++doc_idx) {
        batches.push_back(make_batch_of_one(encode_doc(docs[doc_idx], vocab, bos_token_id)));
    }
    return batches;
}
...
int run_validate(const CliOptions&) {
    ...
    std::vector<BatchTokens> batches;
    batches.push_back(make_batch_of_one(tokens));
    try {
        KernelResult result;
...
int run_benchmark(const CliOptions& options) {
    ...
    const std::vector<BatchTokens> batches = build_single_sequence_batches(docs, steps, vocab, static_cast<int>(uchars.size()));
