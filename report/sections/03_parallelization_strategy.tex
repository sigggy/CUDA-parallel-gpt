...
This creates very little parallel work per operation, especially because the names are short. To increase available parallelism, we batch multiple names together so each kernel operates over many sequences and token positions at once, as shown in Algorithm~\ref{alg:batched}. 
...
With this strategy in place, we will next evaluate how effectively the batched GPU implementation reduces wall-clock time compared to serial and numpy baselines. 