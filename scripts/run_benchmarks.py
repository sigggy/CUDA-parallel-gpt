...
METHODS = ("unbatched_torch", "batched_torch", "serial_cpp", "parallel_cpp")
...
def parse_bool_flag(value: str) -> bool:
    ...
    if normalized in {"f", "false", "0", "n", "no"}:
        return False
    raise argparse.ArgumentTypeError("expected T/F")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        metavar="T|F",
        help="include regime 1 presets (default: T)",
    )
    parser.add_argument(
        "--include-regime-2",
        type=parse_bool_flag,
        default=True,
        metavar="T|F",
        help="include regime 2 presets (default: T)",
    )
    parser.add_argument(
        "--include-regime-3",
        type=parse_bool_flag,
        default=True,
        metavar="T|F",
        help="include regime 3 presets (default: T)",
    )
...
def preset_labels(results: dict[str, object]) -> list[str]:
    preset_order = results.get("preset_order")
    if not isinstance(preset_order, list):
        return []
    return [label for label in preset_order if isinstance(label, str)]
...
def render_preset_info(results: dict[str, object]) -> str:
    presets = results.get("presets")
    if not isinstance(presets, dict):
        return ""
    rows = []
    for label in preset_labels(results):
        details = presets.get(label)
        if not isinstance(details, dict):
            continue
        rows.append(
            "<tr>"
            f"<td>{html_escape(label)}</td>"
            f"<td>{html_escape(details.get('regime'))}</td>"
            f"<td>{html_escape(details.get('n_layer'))}</td>"
            f"<td>{html_escape(details.get('n_embd'))}</td>"
            f"<td>{html_escape(details.get('block_size'))}</td>"
            f"<td>{html_escape(details.get('n_head'))}</td>"
            f"<td>{html_escape(details.get('steps'))}</td>"
            "</tr>"
        )
    return "\n".join(rows)
...
def render_benchmark_rows(results: dict[str, object]) -> str:
    ...
    rows = []
    for preset in preset_labels(results):
        rows.append(
            "<tr class=\"preset-row\">"
            f"<th colspan=\"8\">preset: {html_escape(preset)}</th>"
            "</tr>"
        )
...
def write_html_report(output_path: Path, results: dict[str, object]) -> None:
    selected_regimes = results.get("selected_regimes", {})
    if not isinstance(selected_regimes, dict):
        selected_regimes = {}
...
<body>
  ...
  <p class="meta">Dataset: {html_escape(results.get("dataset", ""))}<br>Default batch size: {DEFAULT_BATCH_SIZE}<br>Regime 1: {html_escape(selected_regimes.get("1", ""))}, Regime 2: {html_escape(selected_regimes.get("2", ""))}, Regime 3: {html_escape(selected_regimes.get("3", ""))}</p>
...
def benchmark_command(method: str, run: BenchmarkRun) -> list[str]:
    ...
    if method in {"batched_torch", "parallel_cpp"}:
...
def run_benchmark_method(method: str, run: BenchmarkRun) -> CommandResult:
    ...
    return run_command([str(BUILD_DIR / method), *command])


def main() -> int:
    args = parse_args()
    all_run_details = run_details()
    selected_runs = tuple(
        run
        for run in BENCHMARK_RUNS
        if (run.regime == 1 and args.include_regime_1)
        or (run.regime == 2 and args.include_regime_2)
        or (run.regime == 3 and args.include_regime_3)
    )
    selected_run_details = {run.label: all_run_details[run.label] for run in selected_runs}
    ...
    results: dict[str, object] = {
        ...
        "selected_regimes": {
            "1": args.include_regime_1,
            "2": args.include_regime_2,
            "3": args.include_regime_3,
        },
        "preset_order": [run.label for run in selected_runs],
        "presets": selected_run_details,
        "build": {},
        "validate": {},
        "benchmarks": [],
    }

    build_commands: list[list[str]] = [["make", "-C", str(ROOT_DIR), "fixtures", "build/serial_cpp"]]
                ...
                results["benchmarks"].append(
                    {
                        "method": method,
                        "preset": preset,
                        "preset_details": selected_run_details[preset],
                        "status": "skipped",
